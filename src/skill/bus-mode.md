...
The wake tier starts a turn on an idle peer so a message does not sit in the mailbox until the owner types "check the Bus". It is best-effort and never a delivery path: the mail is committed to the mailbox first, the wake is attempted once, and delivery still happens through a poll. Nothing about a wake is retried, coalesced, or rate-limited in this cut; one send makes at most one attempt, and the attempt is recorded as a durable `wake` entry (`requested`, `poll_observed`, or `failed:<reason>`) so `storybloq review-stats` can count how often the owner still had to relay. A successful `turn/start` only proves the server accepted a turn; `poll_observed` is written later, by the delivery layer, when the peer's next poll passes the mailbox cursor recorded at wake time.
